...
const ColorsAndSizes = ({ colors, setColors, onStepSubmit, goToPreviousStep }) => {
  ...
  useEffect(function checkColorsStateAndSetDefaultColor() {
    const validColorIndex = colors.findIndex(color => color.value && color.sizes.length && color.images.length);
  ...
  const setDefaultColor = useCallback((index) => {
    const updatedColors = colors.map((color, i) => {
      ...
      <ColorsContainer>
        {
          colors.map((color, index) => {
            return <InputContainer key={index} data-default-styles={color.default && (colors.length >= 2)}>
              <LabelContainer>
